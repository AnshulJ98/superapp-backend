...
const services = [
  ...
  { name: 'analytics-service', base: 'http://analytics-service:3003' },
];
...
async function proxyRequest(req, res) {
  ...
  try {
    const response = await fetch(targetUrl, { method: req.method, headers, body: req.method === 'GET' || req.method === 'HEAD' ? undefined : req });
    ...
    if (body && typeof body.pipe === 'function') {
      ...
      try {
        const writable = new (require('stream').Writable)({
          write(chunk, enc, cb) { res.write(chunk); cb(); }
        });
...
const server = http.createServer((req, res) => {
  if (req.url === '/' && req.method === 'GET') return healthHandler(res);
